...
const PublicInputLocationDropDown: FC<Props> = ({ data, setField, name, value, dropdownType, isError }) => {
    ...
    <div className='w-full mb-5' id={name}>
      ...
      >
        <p className='text-[#A1A5B7] text-base font-normal leading-10'>
          ...
          (data.text !== '' ? data.text :
          dropdownType === FORM_FIELD_TYPE.DROPDOWN_LOCATION ? 'Please select a location'
          : dropdownType === FORM_FIELD_TYPE.DROPDOWN_AREA ? 'Please select an area'
          : dropdownType === FORM_FIELD_TYPE.DROPDOWN_DEPARTMENT ? 'Please select a department' 
          : 'Dropdown list text')}
        </p>
        {dropDownOn ? (
          <GrFormUp
        ...
        {dropDownOn && (
          <div
            className='absolute w-full h-auto
              bg-white border rounded-lg z-10 top-10 left-0 max-h-[200px] overflow-y-auto'
          >
            <div className='m-4'>
              {data?.options?.map((row: any) => {
                return (
                    ...
                    <tbody>
                      <tr
                        ...
                        onClick={() => {
                          ...
                          setField(`${name}`, row.id)
                        }}
                      >
                        <td className='text-gray-500 text-sm hover:font-bold hover:rounded-lg'>
                          <span className='ml-4 w-full h-full '> {row.value}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
export default PublicInputLocationDropDown
