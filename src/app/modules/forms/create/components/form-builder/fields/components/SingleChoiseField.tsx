...
const SingleChoiseField: FC<Props> = ({ value: prevValue, onChange }) => {
  ...
  return (
    <div className='w-full bg-white py-[24px] max-md:py-[15px] px-[32px] max-md:px-[16px]'>
      <input
        ...
        value={value.fieldName === "Single Choice" ? "" : value.fieldName}
        placeholder="Single Choice"
        onChange={(e: any) => {
          setValue({ ...value, fieldName: e.target.value === "" ? "Single Choice" : e.target.value })
          onChange({ ...value, fieldName: e.target.value === "" ? "Single Choice" : e.target.value })
        }}
      />
...
export default SingleChoiseField
