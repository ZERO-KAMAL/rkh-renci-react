...
const ContactNumber: FC<Props> = ({ value: prevValue, onChange }) => {
  const [value, setValue] = useState({ fieldName: 'Contact Number', contact: '', required: false })

  const contactOnChange = (e: any) => {
    const val = { ...value, contact: e.target.value }
    setValue(val)
    onChange(val)
  }
  ...
  return (
    <div className='w-full bg-white py-[24px] max-md:py-[15px] px-[32px] max-md:px-[16px]'>
      <p className='text-[14px] font-medium mb-3'>Contact Number</p>
      <input
